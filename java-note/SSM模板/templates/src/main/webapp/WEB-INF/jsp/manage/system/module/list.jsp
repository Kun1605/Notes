...
<s:url value="/manage/system/module/list" var="listURL"/>
<s:url value="/manage/system/module/detail?backURL=${pagination.encodedCurrentPageURL}" var="detailURL"/>
<s:url value="/manage/system/module/delete?backURL=${pagination.encodedCurrentPageURL}" var="delURL"/>
<!DOCTYPE html>
                        ...
                        <div class="js-stools clearfix">
                            ...
                            <div class="js-stools-container-filters clearfix">
                                <div class="js-stools-field-filter">
                                    <s:message code="moduleInfo.enable" var="enable_option"/>
                                ...
                                <div class="js-stools-field-filter">
                                    <s:message code="moduleInfo.discard" var="discard_option"/>
                        ...
                        <table class="table table-striped" id="userList">
                            ...
                            <tr>
                                ...
                                <th width="1%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.id" order="a.id" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap">
                                    <sg:thlink name="moduleInfo.value" order="a.value" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap">
                                    <sg:thlink name="moduleInfo.path" order="a.path" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.code" order="a.code" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.orderNumber" order="a.order_number" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.pkgName" order="a.pkg_name" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.enable" order="a.enable" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.inletUri" order="a.inlet_uri" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="moduleInfo.discard" order="a.discard" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap left">
                                    <sg:thlink name="moduleInfo.parentId" order="a.parent_id" fullOrdering="${fullordering}"/>
                                </th>
                            ...
                            <tbody>
                            <c:forEach items="${moduleInfoList}" varStatus="status">
                                <c:set var="showModuleInfo" value="${status.current}"/>
                            <tr class="row${status.index % 2}">
                                <td class="nowrap center">
                                    <input type="checkbox" id="cb-${showModuleInfo.id}" name="id"
                                           value="${showModuleInfo.id}" onclick="Joomla.isChecked(this.checked);" />
                                </td>
                                <td class="hidden-phone">${showModuleInfo.id}</td>
                                <td class="nowrap">${showModuleInfo.value}</td>
                                <td class="nowrap">${showModuleInfo.path}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.code}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.orderNumber}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.pkgName}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.enable}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.inletUri}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.discard}</td>
                                <td class="nowrap hidden-phone">${showModuleInfo.parentId}</td>
                                <td class="center nowrap">
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'detail')">
                                        <a class="btn btn-micro active hasTooltip" href="${detailURL}&id=${showModuleInfo.id}"
                                           title="查看 ${showModuleInfo.id}">
                                            <span class="icon-eye-open"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'delete')">
                                        <a class="btn btn-micro active hasTooltip" id="list-delete-${showModuleInfo.id}"
                                           href="${delURL}&id=${showModuleInfo.id}" title="删除 ${showModuleInfo.id}">
        ...
        require(['app/module'], function(module) {
            var config = {
                ...
                message: {
                    ...
                    filter_search_tip: "<sg:message code="JSEARCH_TITLE" ref="moduleInfo.value" />",
