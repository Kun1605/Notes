...
<s:url value="/manage/system/users/department/group/list?departmentId=${departmentInfo.id}" var="listURL"/>
<s:url value="/manage/system/users/department/group/edit?backURL=${backURL}" var="editURL"/>
<s:url value="/manage/system/users/department/group/delete?backURL=${backURL}" var="delURL"/>
<s:url value="/manage/system/users/department/group/member/list" var="memberListURL"/>
            ...
            <div class="span12">
                ...
                <form:form modelAttribute="queryFilter" id="adminForm" name="adminForm" action="${listURL}">
                    ...
                    <div id="j-main-container" class="span10">
                        <legend>${departmentInfo.departmentName} 部门业务组列表</legend>
                        <div class="js-stools clearfix">
                            <c:set var="searchPath" value="filter[groupName]"/>
                        ...
                        <table class="table table-striped" id="userList">
                            ...
                            <tr>
                                ...
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="groupInfo.groupName" order="a.group_name" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="groupInfo.groupMemo" order="a.group_memo" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="groupInfo.status" order="a.status" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="10%" class="nowrap hidden-phone"><s:message code="groupInfo.memberCount"/></th>
                                <th width="20%" class="nowrap hidden-phone">
                                    <sg:thlink name="groupInfo.createTime" order="a.create_time" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="1%" class="nowrap hidden-phone">
                                    <sg:thlink name="groupInfo.id" order="a.id" fullOrdering="${fullordering}"/>
                                </th>
                            ...
                            <tbody>
                            <c:forEach items="${groupInfoList}" varStatus="status">
                                <c:set var="groupInfo" value="${status.current}"/>
                            <tr class="row${status.index % 2}">
                                <td class="nowrap center">
                                    <input type="checkbox" id="cb-${groupInfo.id}" name="id"
                                           value="${groupInfo.id}" onclick="Joomla.isChecked(this.checked);" />
                                </td>
                                <td class="nowrap hidden-phone">${groupInfo.groupName}</td>
                                <td class="nowrap hidden-phone">${groupInfo.groupMemo}</td>
                                <td class="nowrap">
                                    <span class="icon-${groupInfo.status == 1 ? "publish" : "unpublish"}"></span>
                                </td>
                                <td class="nowrap hidden-phone">
                                    <a class="active hasTooltip" href="${memberListURL}?groupId=${groupInfo.id}"
                                       title="成员列表 ${groupInfo.groupName}">
                                        <span class="badge">${groupInfo.memberCount}</span>
                                    </a>
                                </td>
                                <td class="nowrap hidden-phone">${groupInfo.createTimeStr}</td>
                                <td class="hidden-phone">${groupInfo.id}</td>
                                <td class="center nowrap">
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'detail')">
                                        <a class="btn btn-micro active hasTooltip"
                                           href="${detailURL}&id=${groupInfo.id}&departmentId=${departmentInfo.id}"
                                           title="查看 ${groupInfo.groupName}">
                                            <span class="icon-eye-open"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'edit')">
                                        <a class="btn btn-micro active hasTooltip"
                                           href="${editURL}&id=${groupInfo.id}&departmentId=${departmentInfo.id}"
                                           title="编辑 ${groupInfo.groupName}">
                                            <span class="icon-edit"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'delete')">
                                        <a class="btn btn-micro active hasTooltip" id="list-delete-${groupInfo.id}"
                                           href="${delURL}&id=${groupInfo.id}&departmentId=${departmentInfo.id}"
                                           title="删除 ${groupInfo.groupName}">
        ...
        require(['app/module'], function(module) {
            var config = {
                ...
                message: {
                    ...
                    filter_search_tip: "<sg:message code="JSEARCH_TITLE" ref="groupInfo.groupName" />",
