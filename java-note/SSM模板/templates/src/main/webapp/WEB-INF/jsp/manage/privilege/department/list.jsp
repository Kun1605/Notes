...
<s:url value="/manage/privilege/department/list" var="listURL"/>
<s:url value="/manage/privilege/department/companyAuthorize?backURL=${pagination.encodedCurrentPageURL}" var="companyAuthorizeURL"/>
<!DOCTYPE html>
                    ...
                    <div id="j-main-container" class="span10">
                        <div class="js-stools clearfix">
                            ...
                            <div class="js-stools-container-filters clearfix">
                                <div class="js-stools-field-filter">
                                    <s:message code="departmentInfo.status" var="status_option"/>
                        ...
                        <table class="table table-striped" id="userList">
                            ...
                            <tr>
                                ...
                                <th class="nowrap">
                                    <s:message code="departmentInfo.departmentMemo"/>
                                </th>
                                ...
                                <th width="10%" class="nowrap center hidden-phone"><s:message code="departmentInfo.groupCount"/></th>
                                <th width="10%" class="nowrap center hidden-phone"><s:message code="departmentInfo.memberCount"/></th>
                            </tr>
                            </thead>
                            <tfoot>
                            <tr>
                            ...
                            <c:forEach items="${departmentInfoList}" varStatus="status">
                                <c:set var="department" value="${status.current}"/>
                            <tr class="row${status.index % 2}">
                                <td class="center">
                                    <input type="checkbox" id="cb0" name="id" value="${department.id}" onclick="Joomla.isChecked(this.checked);" /></td>
                                <td>
                                    <div class="name">${department.departmentName}</div>
                                </td>
                                <td>
                                    <div class="name">${department.departmentMemo}</div>
                                </td>
                                <sec:authorize access="hasPermission(#commandInfo.module, 'companyAuthorize')">
                                <td class="center">
                                    <a class="btn btn-micro active hasTooltip" href="${companyAuthorizeURL}&id=${department.id}"
                                       title="为 部门 ${department.departmentName} 授权可用公司">
                                        <span class="icon-briefcase"></span></a>
                                </td>
                                </sec:authorize>
                                <td class="center">
                                    <span class="icon-${department.status == 1 ? "publish" : "unpublish"}"></span>
                                </td>
                                <td class="center hidden-phone"><span class="badge">${department.groupCount}</span></td>
                                <td class="center hidden-phone"><span class="badge">${department.memberCount}</span></td>
                            </tr>
                            </c:forEach>
                            </tbody>
                        </table>
        ...
        require(['app/module'], function(module) {
            var config = {
                ...
                message: {
                    ...
                    filter_search_tip: "<sg:message code="JSEARCH_TITLE" ref="departmentInfo.name" />",
