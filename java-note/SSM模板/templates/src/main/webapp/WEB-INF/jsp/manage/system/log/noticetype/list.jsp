...
<s:url value="/manage/system/log/noticetype/list" var="listURL"/>
<s:url value="/manage/system/log/noticetype/detail?backURL=${pagination.encodedCurrentPageURL}" var="detailURL"/>
<s:url value="/manage/system/log/noticetype/edit?backURL=${pagination.encodedCurrentPageURL}" var="editURL"/>
<s:url value="/manage/system/log/noticetype/delete?backURL=${pagination.encodedCurrentPageURL}" var="delURL"/>
<!DOCTYPE html>
                        ...
                        <table class="table table-striped" id="userList">
                            ...
                            <tr>
                                ...
                                <th width="20%" class="left nowrap">
                                    <sg:thlink name="noticeType.code" order="a.code" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="20%" class="nowrap">
                                    <sg:thlink name="noticeType.description" order="a.description" fullOrdering="${fullordering}"/>
                                </th>
                                <th width="1%" class="nowrap">
                                    <sg:thlink name="noticeType.id" order="a.id" fullOrdering="${fullordering}"/>
                                </th>
                            ...
                            <tbody>
                            <c:forEach items="${noticeTypeList}" varStatus="status">
                                <c:set var="noticeType" value="${status.current}"/>
                            <tr class="row${status.index % 2}">
                                <td class="nowrap center">
                                    <input type="checkbox" id="cb-${noticeType.id}" name="id"
                                           value="${noticeType.id}" onclick="Joomla.isChecked(this.checked);" />
                                </td>
                                <td>
                                    <div class="name break-word">
                                        <a href="${editURL}?id=${noticeType.id}" title="编辑通知类型 ${noticeType.code}">
                                        ${noticeType.code}</a>
                                    </div>
                                </td>
                                <td class="nowrap ">${noticeType.description}</td>
                                <td class="nowrap">${noticeType.id}</td>
                                <td class="center nowrap">
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'detail')">
                                        <a class="btn btn-micro active hasTooltip" href="${detailURL}&id=${noticeType.id}"
                                           title="查看 ${noticeType.id}">
                                            <span class="icon-eye-open"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'edit')">
                                        <a class="btn btn-micro active hasTooltip" href="${editURL}&id=${noticeType.id}"
                                           title="编辑 ${noticeType.id}">
                                            <span class="icon-edit"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'delete')">
                                        <a class="btn btn-micro active hasTooltip" id="list-delete-${noticeType.id}"
                                           href="${delURL}&id=${noticeType.id}" title="删除 ${noticeType.id}">
        ...
        require(['app/module'], function(module) {
            var config = {
                ...
                message: {
                    ...
                    filter_search_tip: "<sg:message code="JSEARCH_TITLE" ref="noticeType.id" />",
