...
<s:url value="/manage/system/users/department/group/member/list?groupId=${groupInfo.id}" var="listURL"/>
<s:url value="/manage/system/users/department/group/member/edit?backURL=${backURL}" var="editURL"/>
<s:url value="/manage/system/users/department/group/member/delete?backURL=${backURL}" var="delURL"/>
<s:url value="/manage/system/users/department/group/list?departmentId=${groupInfo.departmentId}" var="groupURL"/>
<!DOCTYPE html>
        ...
        <div class="row-fluid">
            <div class="span12">
                ...
                <form:form modelAttribute="queryFilter" id="adminForm" name="adminForm" action="${listURL}">
                    <legend>${groupInfo.groupName} 业务组成员列表</legend>
                    ...
                    <table class="table table-striped" id="userList">
                        ...
                        <tr>
                            ...
                            <th width="20%" class="nowrap hidden-phone">
                                <sg:thlink name="groupUser.userId" order="a.user_id" fullOrdering="${fullordering}"/>
                            </th>
                            <th width="20%" class="nowrap hidden-phone">
                                <sg:thlink name="groupUser.userName" order="user_name" fullOrdering="${fullordering}"/>
                            </th>
                            <th width="20%" class="nowrap hidden-phone">
                                <sg:thlink name="groupUser.userPosition" order="a.user_position" fullOrdering="${fullordering}"/>
                            </th>
                            <th width="20%" class="nowrap hidden-phone">
                                <sg:thlink name="groupUser.assignTime" order="a.assign_time" fullOrdering="${fullordering}"/>
                            </th>
                            <th width="1%" class="nowrap hidden-phone">
                                <sg:thlink name="groupUser.id" order="a.id" fullOrdering="${fullordering}"/>
                            </th>
                        ...
                        <c:forEach items="${groupUserList}" varStatus="status">
                            <c:set var="groupUser" value="${status.current}"/>
                            <tr class="row${status.index % 2}">
                                <td class="nowrap center">
                                    <input type="checkbox" id="cb-${groupUser.id}" name="id"
                                           value="${groupUser.id}" onclick="Joomla.isChecked(this.checked);" />
                                </td>
                                <td class="nowrap hidden-phone">${groupUser.userId}</td>
                                <td class="nowrap">${groupUser.userName}</td>
                                <td class="nowrap hidden-phone">${groupUser.userPositionDescription}</td>
                                <td class="nowrap hidden-phone">${groupUser.assignTimeStr}</td>
                                <td class="hidden-phone">${groupUser.id}</td>
                                <td class="center nowrap">
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'edit')">
                                        <a class="btn btn-micro active hasTooltip"
                                           href="${editURL}&id=${groupUser.id}&groupId=${groupUser.groupId}"
                                           title="编辑 ${groupUser.id}">
                                            <span class="icon-edit"></span></a>
                                    </sec:authorize>
                                    <sec:authorize access="hasPermission(#commandInfo.module, 'delete')">
                                        <a class="btn btn-micro active hasTooltip"
                                           id="list-delete-${groupUser.id}"
                                           href="${delURL}&id=${groupUser.id}&groupId=${groupUser.groupId}"
                                           title="删除 ${groupUser.id}">
        ...
        require(['app/module'], function(module) {
            var config = {
                ...
                message: {
                    ...
                    filter_search_tip: "<sg:message code="JSEARCH_TITLE" ref="groupUser.userName" />",
